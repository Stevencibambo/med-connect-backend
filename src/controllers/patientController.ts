...
static async getAllPatients (req: Request, res: Response) {
  ...
  static async createPrescription (req: Request, res: Response) {
    try {
      ...
      // Basic validation
      if (!patientId || !data.doctorId || !data.diagnosis || !data.items || data.items.length === 0) {
        return res.status(400).json({
          success: false,
          error: {
  ...
  static async getPatientMedicalVisits (req: Request, res: Response) {
    try {
      const { patientId } = req.params;
      ...
      const result = await PatientService.getPatientMedicalVisits(patientId, pageNum, limitNum, sortBy as string, sortOrder as 'ASC' | 'DESC');
  ...
  static async getAllPrescriptions (req: Request, res: Response) {
    try {
      ...
      const result = await PatientService.getAllPrescriptions(pageNum, limitNum, sortBy as string, sortOrder as 'ASC' | 'DESC');
  ...
  static async getDispensedPrescriptions (req: Request, res: Response) {
    try {
      const { page = 1, limit = 10, sortBy = 'created_at', sortOrder = 'DESC' } = req.query;
      const pageNum = parseInt(page as string);
      const limitNum = parseInt(limit as string);

      const result = await PatientService.getDispensedPrescriptions(pageNum, limitNum, sortBy as string, sortOrder as 'ASC' | 'DESC');
  ...
  static async updatePrescriptionStatus (req: Request, res: Response) {
    ...
    try {
      ...
      const result = await PrescriptionService.dispensePrescription({...data, prescriptionId: prescriptionId});
