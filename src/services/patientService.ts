import { Patient, User, MedicalVisit, Prescription, PrescriptionItem, UserRole, QRCode } from '../models';
...
static async getAllPatients (limit: number = 10, offset: number = 0): Promise<{ patients: PatientProfile[], total: number }> {
  ...
  static async getPatientMedicalVisits(patientId: string, page: number = 1, limit: number = 10, sortBy: string = 'visitDate', sortOrder: 'ASC' | 'DESC' = 'DESC'): Promise<{ visits: MedicalVisitSummary[], total: number }> {
    const offset = (page - 1) * limit;

    const { count, rows: visits } = await MedicalVisit.findAndCountAll({
      where: { patientId },
      include: [
        {
          model: Doctor,
  ...
  static async getAllPrescriptions(page: number = 1, limit: number = 10, sortBy: string = 'createdAt', sortOrder: 'ASC' | 'DESC' = 'DESC'): Promise<{ prescriptions: any[], total: number }> {
    const offset = (page - 1) * limit;
    ...
    const prescriptionData = prescriptions.map(prescription => ({
      ...
      qrCode: (prescription as any).qrCode,
      createdAt: prescription.createdAt,
    }));
  ...
  static async getDispensedPrescriptions(page: number = 1, limit: number = 10, sortBy: string = 'createdAt', sortOrder: 'ASC' | 'DESC' = 'DESC'): Promise<{ prescriptions: any[], total: number }> {
    const offset = (page - 1) * limit;

    const { count, rows: prescriptions } = await Prescription.findAndCountAll({
      where: { status: PrescriptionStatus.FULFILLED }, // Only fetch prescriptions with status 'pending'
      include: [
        {
          model: Patient,
          as: 'patient',
        ...
        {
          model: PrescriptionDispensed,
          as: 'dispensed',
          attributes: ['pharmacistId', 'dispensedAt']
        },
        {
          model: QRCode,
          as: 'qrCode',
    ...
    const prescriptionData = prescriptions.map(prescription => ({
      ...
      patient: (prescription as any).patient,
      qrCode: (prescription as any).qrCode,
      dispensingTimeline: (prescription as any).dispensed,
      createdAt: prescription.createdAt,
    }));
